...
function progressTime(game: GameProps, timestamp: number) {
...
function adjustDifficulty(game: GameProps) {
...
function renderGame(game: GameProps, canvas, timestamp: number) {
...
function drawBackground(ctx, canvasWidth: number, canvasHeight: number) {
...
function drawCell(ctx, { cell, row, column, cellWidth, cellHeight }: DrawCellProps) {
...
interface DrawScoringProps {
  score: number;
  level: number;
  canvasWidth: number;
}

function drawScoring(ctx, { score, level, canvasWidth }: DrawScoringProps) {
  const margin = 5;
  const fontSize = 22;
  drawText(ctx, {
    text: `Level ${level}`,
...
function drawText(ctx, { text, x, y, font, textAlign }: DrawTextProps) {
  ctx.font = font;
